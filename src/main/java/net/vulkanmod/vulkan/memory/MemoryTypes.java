...
public class MemoryTypes {
    ...
    public static void createMemoryTypes() {
        for (
            int i = 0;
            i < DeviceManager.memoryProperties.memoryTypeCount();
            ++i
        ) {
            VkMemoryType memoryType =
                DeviceManager.memoryProperties.memoryTypes(i);
            VkMemoryHeap heap = DeviceManager.memoryProperties.memoryHeaps(
            ...
            if (
                ...
                else GPU_MEM = new DeviceLocalMemory(memoryType, heap);
            }
    ...
    public static class DeviceLocalMemory extends MemoryType {
        ...
        public void copyToBuffer(
            ...
            DeviceManager.getTransferQueue().copyBufferCmd(
                stagingBuffer.getId(),
        ...
        public long copyBuffer(Buffer src, Buffer dst) {
            if (dst.getBufferSize() < src.getBufferSize()) {
                throw new IllegalArgumentException(
                    "dst size is less than src size."
                );
            }

            return DeviceManager.getTransferQueue().copyBufferCmd(
                src.getId(),
                0,
                dst.getId(),
                0,
    ...
    // Used on UMA/integrated GPUs: DEVICE_LOCAL heap is also HOST_VISIBLE.
    // Inherits direct-memcpy upload path from MappableMemory, so no staging buffer
    // is needed. HOST_COHERENT ensures CPU writes are immediately GPU-visible without
    // manual vmaFlushAllocation calls.
    ...
    // Kept for discrete-GPU BAR (resizable BAR / 256 MB aperture) where the first
    // DEVICE_LOCAL heap is HOST_VISIBLE but not coherent. Not used on UMA.
    static class DeviceMappableMemory extends MappableMemory {

        DeviceMappableMemory(
            VkMemoryType vkMemoryType,
            VkMemoryHeap vkMemoryHeap
        ) {
            super(Type.DEVICE_LOCAL, vkMemoryType, vkMemoryHeap);
