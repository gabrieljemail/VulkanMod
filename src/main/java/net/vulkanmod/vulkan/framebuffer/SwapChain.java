...
public class SwapChain extends Framebuffer {
    ...
    public void recreate() {
        ...
        if (!DYNAMIC_RENDERING) {
            this.FBO_map.forEach((pass, framebuffers) ->
                Arrays.stream(framebuffers).forEach(id ->
                    vkDestroyFramebuffer(getVkDevice(), id, null)
                )
            );
            this.FBO_map.clear();
        }
    ...
    private void createSwapChain() {
        try (MemoryStack stack = stackPush()) {
            ...
            DeviceManager.SurfaceProperties surfaceProperties =
                DeviceManager.querySurfaceProperties(
                    device.getPhysicalDevice(),
                    stack
                );

            VkSurfaceFormatKHR surfaceFormat = getFormat(
                surfaceProperties.formats
            );
            ...
            if (extent.width() == 0 && extent.height() == 0) {
                if (this.swapChainId != VK_NULL_HANDLE) {
                    this.swapChainImages.forEach(image ->
            ...
            int requestedImages =
                surfaceProperties.capabilities.minImageCount() + 1;
            if (
                surfaceProperties.capabilities.maxImageCount() > 0 &&
                requestedImages > surfaceProperties.capabilities.maxImageCount()
            ) {
                requestedImages =
                    surfaceProperties.capabilities.maxImageCount();
            }
            ...
            createInfo.preTransform(
                surfaceProperties.capabilities.currentTransform()
            );
    ...
    protected long getFramebufferId(RenderPass renderPass) {
        long[] framebuffers = this.FBO_map.computeIfAbsent(
            renderPass.id,
            renderPass1 -> createFramebuffers(renderPass)
        );
    ...
    private VkSurfaceFormatKHR getFormat(
        VkSurfaceFormatKHR.Buffer availableFormats
    ) {
    ...
    private int getPresentMode(IntBuffer availablePresentModes) {
        if (vsync) {
            // Look for Adaptive VSync.
            // This usually improves frame pacing with drivers that support it.
            for (int i = 0; i < availablePresentModes.capacity(); i++) {
                if (
                    availablePresentModes.get(i) ==
                    VK_PRESENT_MODE_FIFO_RELAXED_KHR
                ) {
                    return VK_PRESENT_MODE_FIFO_RELAXED_KHR;
                }
            }
            // Fall back to standard VSync if the driver lacks relaxed mode
        ...
        Initializer.LOGGER.warn(
            "Requested mode not supported: " +
                getDisplayModeString(requestedMode) +
    ...
    private static int checkPresentMode(int... requestedModes) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            var a = DeviceManager.querySurfaceProperties(
